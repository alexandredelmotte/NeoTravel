...
export type DonneesMail = {
  ...
  destination: string;   // ex : "Lyon"
  date: string;          // ex : "15 juillet 2026"
  passagers: string | number; // ex : 45
  reference: string;     // ex : "NEO-2026-0847"
  prix?: string;         // ex : "1 835 € TTC" (surtout pour le devis)
  lienDevis?: string;    // lien vers le devis (sinon "#")
};

type Modele = { subject: string; html: string };

// --- petits utilitaires de mise en forme ---

const COULEURS = { nuit: "#1D3557", bleu: "#5784BA", ciel: "#9AC8EB", fond: "#F6F8FB", texte: "#26302B", muted: "#6A7A90", ligne: "#E4E0D4" };
...
function bouton(texte: string, lien: string): string {
...
function recap(d: DonneesMail): string {
  const lignes: [string, string][] = [
    ["Trajet", `${d.depart} → ${d.destination}`],
    ["Date", d.date],
    ["Passagers", String(d.passagers)],
  ];
  if (d.prix) lignes.push(["Prix estimé", d.prix]);
...
// L'enveloppe commune à tous les mails (en-tête + pied de page)
function shell(preheader: string, contenu: string): string {
  return `<!doctype html>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${COULEURS.fond};padding:24px 12px;">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background:#ffffff;border-radius:14px;overflow:hidden;">
        <tr><td style="background:${COULEURS.nuit};padding:20px 28px;font-family:Arial,sans-serif;font-size:18px;font-weight:bold;">
          <span style="color:${COULEURS.ciel};">✦ Neo</span><span style="color:#ffffff;">travel</span>
        </td></tr>
        <tr><td style="padding:30px 28px;">${contenu}</td></tr>
        <tr><td style="padding:18px 28px;background:${COULEURS.fond};color:${COULEURS.muted};font-size:12px;line-height:1.6;font-family:Arial,sans-serif;">
...
// 1) Envoi du devis
export function mailDevis(d: DonneesMail): Modele {
...
// 2) Première relance (J+2 si urgent, sinon J+3)
export function mailRelance1(d: DonneesMail): Modele {
  const lien = d.lienDevis || "#";
  const contenu =
    p(`Bonjour ${d.nom},`) +
...
// 3) Deuxième et dernière relance (J+7)
export function mailRelance2(d: DonneesMail): Modele {
  const lien = d.lienDevis || "#";
  const contenu =
    p(`Bonjour ${d.nom},`) +
